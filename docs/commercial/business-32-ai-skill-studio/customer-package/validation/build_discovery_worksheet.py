"""Build Business32_Skill_Discovery_Worksheet.docx (max 3 pages)."""
import os
...
para("제안검토용 DRAFT · 합성 샘플 기반 · 실제 파일·개인정보는 넣지 마세요", size=8, color=(0x6B, 0x72, 0x80), space_after=8)

para("기관·팀 (합성 예시: 가상 기관명 A / 홍보팀)", size=9, color=(0x6B, 0x72, 0x80))
answer_line()

for i, (question, hint) in enumerate(QUESTIONS, start=1):
    p = doc.add_paragraph()
    run = p.add_run(checkbox() + "Q%d. %s" % (i, question))
    run.bold = True
    run.font.size = Pt(10)
...
para("작성 후에는 업무명·단계·소요시간·검토 구조만 확인하며, 실제 견적서·내부 문서·개인정보를 수집하지 않습니다.",
     size=8, color=(0x6B, 0x72, 0x80), space_after=0)
para("사람 검토와 승인이 필수입니다. AI가 자동 승인하지 않습니다. 모든 예시는 합성입니다.",
     size=8, color=(0x2F, 0x85, 0x5A), space_after=0)
