...
- `DIRECT_PRODUCTION_DEPLOYMENT_AND_ROLLBACK_POLICY.md` — canonical deployment policy: direct Production by default, Preview optional, rollback mandatory.
...
- do not mix UI, UX, authentication, databases, live models, and deployment in one issue.

## Deployment default

`DIRECT_PRODUCTION_AFTER_EXPLICIT_AUTHORIZATION`

Preview:
`OPTIONAL_NOT_REQUIRED`

Rollback:
`MANDATORY_PREPARED_GATE`

Issue #154 stays open while AI Revenue Lab continues creating or revisiting Businesses. Each Business is tracked separately across UI, UX, and backend states.
