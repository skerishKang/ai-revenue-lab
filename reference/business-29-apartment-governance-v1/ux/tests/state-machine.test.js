...
check("quorum supplement and recheck", function () {
  var m = SM.createMachineAt(fixture, "quorum-incomplete");
  assert.strictEqual(m.allowed("confirmQuorum"), false, "direct incomplete → recorded is blocked");
  assert.throws(function () {
    m.apply("confirmQuorum", { attendance: 11, threshold: 10, manualConfirm: true });
  }, "quorum-incomplete cannot skip attendance supplement");
...
/* 20b. guide boundary */
check("guide boundary preserves Phase 1 and synthetic-only separation", function () {
  var guide = fs.readFileSync(path.join(workspace, "..", "guide.html"), "utf8");
  [
    "Phase 1 root identity and surface",
    "synthetic governance-ledger sandbox/reference",
    "no real resident data",
    "real governance data",
    "real litigation data",
    "no legal judgement",
    "no real electronic voting",
    "no backend",
    "authentication",
    "database",
    "quorum-incomplete → attendance-open → attendance supplement → manual recheck → quorum-recorded or quorum-incomplete",
    "Direct <code>quorum-incomplete → quorum-recorded</code> is forbidden"
  ].forEach(function (marker) {
    assert.ok(guide.indexOf(marker) !== -1, "missing guide marker: " + marker);
  });
});
...
check("allowed-scope check (only canonical Phase2 workspace changed)", function () {
