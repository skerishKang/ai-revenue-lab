(() => {
  ...
  const benefitLabel = signal => {
    if (verifiedExpiring(signal)) return '기간 한정 무료';
    if (signal.dealType === 'RECURRING_CREDIT') return '무료 크레딧';
    if (signal.dealType === 'FREE_MODEL') return '무료 모델';
    return '상시 무료';
  };
  ...
  function freeCard(signal, index) {
    return `<article class="radar-free-card ${index === 0 ? 'is-wide' : ''}">
      ...
      <div class="radar-free-copy">
        <div class="radar-meta"><b>상시 무료</b><span>${esc(signal.provider)}</span></div>
  ...
  function expiringBlock() {
    if (!expiring.length) {
      return `<div class="radar-expiry-empty"><span>종료 임박</span><strong>지금은 공식 종료일이 확인된 혜택이 없습니다.</strong><p>날짜를 추측해서 긴급함을 만들지 않습니다. 공식 출처에서 종료일이 확인된 혜택만 여기에 올라옵니다.</p></div>`;
    }
    return expiring.map(signal => `<article class="radar-expiry-card">
      <span>종료 임박</span><strong>${esc(signal.freeLabel)}</strong><time>${esc(signal.expiresAt)}</time><button type="button" data-radar-open="${esc(signal.id)}">보기</button>
    </article>`).join('');
  }
  ...
  radar.innerHTML = `
    <div class="radar-masthead">
      ...
      <span>${signals.length}개 확인 경로 · 수동 큐레이션</span>
    </div>

    <div class="radar-rule"><span>지금 무료</span><time>${new Date().toISOString().slice(0, 10)}</time></div>

    <div class="radar-lead" id="radar-live">
      ${featureCard(featured)}
      <aside class="radar-board" aria-label="오늘의 무료 보드">
        <header><span>오늘의 보드</span><strong>지금 바로 볼 것</strong></header>
    ...
    <section class="radar-always" id="always-free">
      <header class="radar-section-head"><div><span>상시 무료</span><h2>오늘만이 아니라,<br>계속 무료인 것들.</h2></div><p>무료 티어·무료 모델·반복 크레딧 중 상시 이용 조건이 확인된 경로를 모았습니다.</p></header>
      <div class="radar-free-grid">${evergreen.map(freeCard).join('')}</div>
    </section>

    <section class="radar-checked" id="just-checked">
      <header class="radar-section-head"><div><span>최근 확인</span><h2>광고 문구보다<br>확인 날짜를 봅니다.</h2></div><p>사용 조건이 바뀌기 쉬운 정보라서, 무엇을 언제 공식 페이지에서 확인했는지 같이 보여줍니다.</p></header>
      <div class="radar-check-list">
        ${sortedChecked.map(signal => `<button type="button" data-radar-open="${esc(signal.id)}"><time>${esc(signal.verifiedAt || '—')}</time><span>${esc(signal.provider)}</span><strong>${esc(signal.freeLabel)}</strong><em>${official(signal) ? '공식' : '검토'}</em></button>`).join('')}
      </div>
      ${pending.length ? `<div class="radar-pending"><span>확인 중</span>${pending.map(signal => `<p><b>${esc(signal.provider)}</b> · ${esc(signal.pending.label)} <em>${esc(signal.pending.state)}</em></p>`).join('')}<small>확인되지 않은 종료일이나 프로모션 문구는 지금 무료/종료 임박으로 승격하지 않습니다.</small></div>` : ''}
    </section>
  `;
