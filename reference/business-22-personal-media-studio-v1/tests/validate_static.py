...
def main() -> int:
    ...
    css = "\n".join(
        ...
        for name in ["styles.css", "styles-editions.css", "styles-responsive.css", "motion-timing.css"]
    )
    ...
    checks.append(
        result(
            "exact seven states",
            len(found_states) == 7 and set(found_states) == set(STATES),
            f"found={found_states}",
        )
    )
    for state in STATES:
    ...
    review_timing = re.search(
        r"\.step-review\s*\{\s*animation:\s*relayReview\s+([\d.]+)ms\s+([\d.]+)ms\s+both",
        timing_css,
    )
    computed_end = None if not review_timing else float(review_timing.group(1)) + float(review_timing.group(2))
    checks.append(
        result(
            "computed final motion end",
            computed_end is not None and 680 <= computed_end <= 760,
            f"computedFinalEndMs={computed_end}",
        )
    )
    ...
    korean_labels = html.count("합성")
    checks.append(
        result(
            "synthetic labels",
            korean_labels >= 10 and "SYNTHETIC" in html,
            f"Korean labels={korean_labels}",
        )
    )
    checks.append(
        result(
            "UI_ONLY limitations",
            "업로드·생성·편집·저장·내보내기·게시 기능은 구현되지 않았습니다" in html,
            "phase limitation visible",
        )
    )
    ...
    report = {
        ...
        "computedFinalEndMs": computed_end,
        "passed": all(check["passed"] for check in checks),
        "checks": checks,
    }
    EVIDENCE.mkdir(parents=True, exist_ok=True)
    (EVIDENCE / "static-validation.json").write_text(
