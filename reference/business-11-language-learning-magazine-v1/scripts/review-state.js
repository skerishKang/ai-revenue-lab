(() => {
  ...
  function normalizedState(value) {
    return stateNames.includes(value) ? value : "cover";
  }

  function showState(nextState, { updateHash = true, focusPanel = false } = {}) {
    ...
    if (updateHash && location.hash !== `#${state}`) {
  ...
  function setEchoExpanded(expanded) {
    window.clearTimeout(echoTimer);
    echoStage.classList.remove("is-echoing");
    echoStage.classList.toggle("is-expanded", expanded);
    echoTrigger.classList.toggle("is-expanded", expanded);
    echoTrigger.setAttribute("aria-expanded", String(expanded));
    echoReplay.setAttribute("aria-busy", "false");
  }

  function replayEcho() {
    const stableScrollY = window.scrollY;
    window.clearTimeout(echoTimer);
    echoStage.classList.remove("is-expanded", "is-echoing");
    echoTrigger.classList.remove("is-expanded");
    echoTrigger.setAttribute("aria-expanded", "false");
    echoReplay.setAttribute("aria-busy", "true");

    if (reducedMotion.matches) {
      setEchoExpanded(true);
  ...
  tabs.forEach((tab, index) => {
    ...
    tab.addEventListener("keydown", (event) => {
      ...
      showState(tabs[nextIndex].dataset.state);
    });
  });

  echoTrigger.addEventListener("click", () => {
    setEchoExpanded(!echoStage.classList.contains("is-expanded"));
  });
  echoReplay.addEventListener("click", replayEcho);
  echoReset.addEventListener("click", () => setEchoExpanded(false));

  window.addEventListener("hashchange", () => showState(location.hash.slice(1), { updateHash: false }));
