#!/usr/bin/env python3
"""Fail-closed guards for approved Cloudflare Pages source-directory migration."""
...
def _metadata_text(metadata: dict[str, Any], field: str) -> str:
    ...
    text = str(value or "")
    if not text:
        raise ValidationError(f"repository metadata is missing {field}")
    return text
...
def build_patch_payload(
    ...
    """Build a bounded PATCH body that preserves all verified source identity fields.

    Cloudflare keeps repository identity and source-path controls under the same
    nested source.config object. To avoid depending on unspecified nested PATCH
    omission semantics, this payload re-sends the small allowlisted identity
    contract while changing only root_dir/path_includes values.
    """
    ...
    return {
        ...
        "source": {
            "type": "github",
            "config": {
                "owner": _metadata_text(repository_metadata, "owner_login"),
                "owner_id": _metadata_text(repository_metadata, "owner_id"),
                "repo_name": _metadata_text(repository_metadata, "repository_name"),
                "repo_id": _metadata_text(repository_metadata, "repository_id"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    payload.add_argument("--production-branch", required=True)
    payload.add_argument("--repository-metadata-file", required=True, type=Path)
...
def main() -> int:
    ...
    try:
        if args.command == "contract":
            ...
            metadata = json.loads(args.repository_metadata_file.read_text(encoding="utf-8"))
            if not isinstance(metadata, dict):
                raise ValidationError("repository metadata must be a JSON object")
            write_json(
                args.output,
