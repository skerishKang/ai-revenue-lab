...
# ── Start Screen (Alpha) ───────────────────────────────────────────────
_t("start.title", "모델을 몰라도 됩니다. 하려는 일과 예산만 알려주세요.", "You don't need to know the model. Tell us the task and budget.")
_t("start.description", "한국어로 목적을 적으면 가격, 속도, 처리 위치를 비교해 맞는 모델 경로를 보여드립니다. 익숙해지면 직접 모델과 Provider를 고를 수 있습니다.", "Enter your task in Korean and we'll show the best model path by price, speed, and locality.")
_t("start.prompt_label", "한국어로 무엇을 도와드릴까요?", "What can I help you with in Korean?")
_t("start.prompt_placeholder", "예: 한국어로 세 문장으로 설명해줘", "e.g. Explain in Korean in three sentences")
_t("start.preset_general", "일반 대화", "General chat")
_t("start.preset_korean", "한국어 요약", "Korean summary")
_t("start.preset_code", "코드 수정", "Code fix")
_t("start.preset_document", "문서 정리", "Document cleanup")
_t("start.preset_batch", "대량 분류", "Batch classification")
_t("start.preset_private", "비공개 작업", "Private task")
_t("start.optimize_balanced", "균형 (비용·속도·한국어)", "Balanced (cost·speed·Korean)")
_t("start.optimize_cost", "비용 절약", "Cost saving")
_t("start.optimize_latency", "응답 속도", "Response speed")
_t("start.optimize_korean", "한국어 적합도", "Korean suitability")
_t("start.external_fallback", "외부 fallback 허용", "Allow external fallback")
_t("start.send", "보내기", "Send")
_t("start.sending", "전송 중...", "Sending...")
_t("start.route_title", "이번 요청에 맞는 선택", "Best match for this request")
_t("start.route_kicker", "예상 경로", "Expected route")
_t("start.route_ready", "프롬프트를 입력하고 보내기를 누르세요.", "Enter a prompt and click send.")
_t("start.mode_auto", "자동", "Auto")
_t("start.mode_manual", "수동", "Manual")
_t("start.mock_label", "모의 응답 · 실제 Provider 호출 없음", "Mock response - no real Provider call")
_t("start.live_label", "실제 Provider 응답", "Live Provider response")
_t("start.key_missing", "Provider 연결 없음 — Live 모드 설정을 확인하십시오", "No Provider connection - check Live mode settings")
_t("start.provider", "Provider", "Provider")
_t("start.model", "모델", "Model")
_t("start.reason", "선택 이유", "Reason")
_t("start.request_status", "요청 상태", "Request status")
_t("start.cost_label", "이번 요청 예상 비용", "Estimated cost for this request")
_t("start.cost_notice", "실제 비용은 Provider 계정과 계약에 따라 별도로 청구됩니다. Business 14는 이 파일럿에서 사용료를 청구하지 않습니다.", "Actual costs are billed separately by your Provider contract. Business 14 does not charge for this pilot.")
_t("start.response_title", "응답", "Response")
_t("start.request_id", "Request ID", "Request ID")
_t("start.tokens", "토큰", "Tokens")
_t("start.estimated_cost", "예상 비용", "Estimated cost")
_t("start.latency", "지연 시간", "Latency")
_t("start.no_key_live", "Live 모드에서 Provider 자격 증명이 필요합니다. platform-owned Provider 키 또는 keyless 라우트를 확인하십시오.", "Live mode requires a Provider credential: a platform-owned Provider key or a keyless route.")
_t("start.no_safe_route", "안전한 라우팅 경로를 찾을 수 없습니다.", "No safe routing path found.")
_t("start.cost_calculated", "configured estimate (예상치)", "configured estimate")
_t("start.cost_unknown", "계산 불가 (price 미공개)", "Cannot calculate (price not published)")
...
_t("nav.start", "시작", "Start")
_t("nav.models", "모델 카탈로그", "Models")
_t("nav.plans", "요금제", "Plans")
_t("nav.usage", "사용량", "Usage")
_t("nav.developer", "개발자", "Developer")
