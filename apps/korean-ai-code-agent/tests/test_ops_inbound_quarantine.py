...
class InboundQuarantineTests(unittest.TestCase):
    ...
    def test_release_requires_attachment_policy_but_still_only_creates_review_ref(self):
        attachment = AttachmentMetadata("att_1", "quote.pdf", "application/pdf", 10, PDF_SHA)
        store = InMemoryInboundQuarantine()
        store.quarantine(message(attachments=(attachment,)), provider_event_ref="provider_evt_1", now=NOW)
        released = store.release_for_review("provider_evt_1", review_ref="intake_review_1", now=NOW + timedelta(seconds=10))
    ...
    def test_rejected_attachment_blocks_release(self):
        ...
        with self.assertRaises(ContractError):
            store.release_for_review("provider_evt_1", review_ref="review_1", now=NOW + timedelta(seconds=1))

    def test_oversized_attachment_blocks_release(self):
