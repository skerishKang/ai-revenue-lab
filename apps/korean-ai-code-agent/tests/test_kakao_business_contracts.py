...
class KakaoBusinessContractTests(unittest.TestCase):
    def test_repository_non_authority_constants_fail_closed(self) -> None:
        self.assertTrue(OFFICIAL_KAKAO_BUSINESS_ONLY)
        self.assertFalse(PERSONAL_KAKAOTALK_SESSION_AUTOMATION_SUPPORTED)
        self.assertFalse(ADDRESS_BOOK_SCRAPING_SUPPORTED)
        self.assertTrue(ALIMTALK_APPROVED_TEMPLATE_REQUIRED)
        self.assertTrue(MIXED_CONTENT_DEFAULTS_ADVERTISING)
        self.assertEqual(ADVERTISING_QUIET_HOURS_START_KST, 21)
        self.assertEqual(ADVERTISING_QUIET_HOURS_END_KST, 8)
        self.assertFalse(BULK_UNAPPROVED_SEND_SUPPORTED)
        self.assertFalse(RAW_KAKAO_CREDENTIAL_IN_B54)
        self.assertFalse(REAL_KAKAO_BUSINESS_CONFIGURED)
        self.assertFalse(REAL_KAKAO_SEND_CONFIGURED)
        self.assertFalse(PRODUCTION_MUTATION_SUPPORTED)

    def test_business_binding_is_exact_and_does_not_expose_recipients(self) -> None:
        s = scope(KakaoBusinessProduct.ALIMTALK, KakaoBusinessProduct.BRAND_MESSAGE)
        self.assertTrue(s.authorizes(
            binding_ref="binding-kakao",
            workspace_ref="ws-1",
            product=KakaoBusinessProduct.ALIMTALK,
            recipient_ref="recipient-1",
        ))
        self.assertFalse(s.authorizes(
            binding_ref="binding-kakao",
            workspace_ref="ws-1",
        ...
        safe = s.safe_dict()
        self.assertEqual(safe["recipient_count"], 2)
        self.assertNotIn("allowed_recipient_refs", safe)
        self.assertFalse(safe["raw_credential_present"])

    def test_alimtalk_requires_information_purpose_and_exact_template(self) -> None:
    ...
    def test_alimtalk_preflight_allows_only_approved_exact_template_and_keys(self) -> None:
        ...
        self.assertIs(decision, KakaoOutboundPreflightDecision.ALLOW)
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.TEMPLATE_NOT_APPROVED,
        )
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.TEMPLATE_MISMATCH,
        )

    def test_alimtalk_rejects_promotional_content_even_with_approved_template(self) -> None:
        m = material(KakaoBusinessProduct.ALIMTALK)
        self.assertIs(
            kakao_outbound_preflight(
                scope=scope(KakaoBusinessProduct.ALIMTALK),
            ...
            KakaoOutboundPreflightDecision.PURPOSE_MISMATCH,
        )

    def test_advertising_requires_active_eligibility_required_fields_and_allowed_window(self) -> None:
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.ALLOW,
        )
        self.assertIs(
            kakao_outbound_preflight(
                scope=s,
            ...
            KakaoOutboundPreflightDecision.CONSENT_REQUIRED,
        )
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.COMPLIANCE_REQUIRED,
        )
        self.assertIs(
            kakao_outbound_preflight(
                scope=s,
            ...
            KakaoOutboundPreflightDecision.QUIET_HOURS,
        )

    def test_channel_message_requires_channel_friend_eligibility(self) -> None:
    ...
    def test_advertising_window_is_08_to_before_21_kst(self) -> None:
        self.assertTrue(compliance(datetime(2026, 9, 2, 23, 0, tzinfo=UTC)).advertising_window_allowed())
        self.assertTrue(compliance(datetime(2026, 9, 3, 11, 59, tzinfo=UTC)).advertising_window_allowed())
        self.assertFalse(compliance(datetime(2026, 9, 3, 12, 0, tzinfo=UTC)).advertising_window_allowed())
        self.assertFalse(compliance(datetime(2026, 9, 2, 22, 59, tzinfo=UTC)).advertising_window_allowed())

    def test_cs_talk_requires_exact_user_initiated_active_session(self) -> None:
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.CS_SESSION_REQUIRED,
        )

    def test_material_changes_invalidate_approval_and_intent(self) -> None:
        ...
        self.assertNotEqual(original.material_fingerprint, changed.material_fingerprint)
        self.assertIs(
            kakao_outbound_preflight(
            ...
            KakaoOutboundPreflightDecision.MATERIAL_CHANGED,
        )

    def test_wrong_connector_and_version_fail_closed(self) -> None:
        m = material(KakaoBusinessProduct.BRAND_MESSAGE)
        a = approval(m)
        base = intent(m)
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.WRONG_CONNECTOR_OR_TOOL,
        )
        ...
        self.assertIs(
            ...
            KakaoOutboundPreflightDecision.VERSION_BINDING_MISMATCH,
        )

    def test_delivery_receipt_correlates_provider_request_and_write_intent(self) -> None:
        m = material(KakaoBusinessProduct.ALIMTALK)
        i = intent(m)
        ...
        self.assertFalse(safe["model_text_counts_as_delivery"])
        self.assertFalse(safe["provider_acceptance_equals_final_delivery"])

    def test_failed_delivery_requires_failure_reason_and_exact_request(self) -> None:
        with self.assertRaises(ContractError):
            KakaoDeliveryEvidence(
                request_ref="request-1",
                provider_status_ref="status-1",
