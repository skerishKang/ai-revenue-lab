...
def _ref(value: str | None, field_name: str, *, required: bool = True) -> str | None:
    ...
    if not value:
        if required:
            raise ContractError(f"{field_name} is required")
        return None
    # Two separate contracts, in this order. The first asks whether the value carries
    # credential material, which is the canonical detector's question; the second asks
    # whether it is a well-formed reference at all, which is not. A value can fail either
    # one alone, and the messages are how callers tell them apart.
