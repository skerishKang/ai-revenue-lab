"""#2937/#2962/#2972/#2989/#2825: bounded native HWPX Skill facade.

Surfaces: inspect / read / validate / create / edit / insert_table /
template_fill / insert_image.
...
``hwpx.insert_image`` (#2825) embeds one image into an existing package that
already carries a manifest, over the same reserved edit capability::

    caller filename + HWPX bytes + one bounded image request
    -> inspect_file()                            (common gate, HWPX_CANDIDATE)
    -> insert_hwpx_image()                       (single Core image authority)
    -> inspect_file() / hwpx_validate() / hwpx_read() on the output
    -> read_hwpx_section_facts() readback of the inserted picture
    -> member-sequence and non-target payload preservation proof
    -> bounded insert_image receipt + in-memory artifact

It adds no second image decoder, XML parser or archive writer: the request is a
section address plus image bytes plus an optional HWPUNIT draw extent, the
canonical PNG decision and every splice belong to Core, and a caller never
supplies a manifest id, relationship id, member name, member path or namespace.
...
- Core ``insert_hwpx_image`` remains the single HWPX image insertion authority:
  it alone decides the canonical PNG form, allocates the ``BIN####`` manifest
  item id and performs the two byte splices, and it delegates decoding to
  ``image_helpers`` and picture serialization to the canonical section producer.
  ``hwpx.insert_image`` composes it and adds no decoder, parser or archive
  writer of its own (``SINGLE_HWPX_IMAGE_INSERTION_AUTHORITY=YES``,
  ``SECOND_HWPX_IMAGE_AUTHORITY=0``,
  ``SECOND_IMAGE_DECODER_AUTHORITY=0``).
- ``IMAGE_INSERT_SUCCESS_REQUIRES_READBACK=YES``: inserted bytes are reported as
  success only after the common gate admitted the output as ``HWPX_CANDIDATE``,
  the existing validate and read authorities accepted it, the inserted picture
  read back through the single Core fact layer at the requested section with the
  requested draw extent, and every non-target member payload was proven
  byte-identical to the supplied package.
...
``CAPABILITY_HWPX_CREATE``, ``hwpx.edit`` and the bounded
``hwpx.insert_table`` and ``hwpx.insert_image`` facades under
``CAPABILITY_HWPX_EDIT``, and ``hwpx.template_fill`` under the already
reserved ``CAPABILITY_HWPX_TEMPLATE_FILL``. No Skill Registry capability was
added or renamed for image insertion.
...
rectangular Core table block at an exact section/block position,
``hwpx.insert_image`` embeds one PNG-canonical image into a manifest-carrying
package at an exact section address, and ``hwpx.template_fill`` is a foundation
over the smallest deterministic placeholder grammar defined above. None
implements paragraph/section insert or delete, existing-table edit/delete,
row/column mutation, image replace/delete/resize/crop/rotate, direct JPEG
embedding, style or layout editing, or legacy HWP conversion; none produces
...
__all__ = [
    ...
    "REASON_INSERT_IMAGE_OUTPUT_COUNT_DRIFT",
    ...
    "VALIDATION_STATUS_INSERT_IMAGE_NOT_RUN",
    "VALIDATION_STATUS_INSERT_IMAGE_OK",
    "VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_MISMATCH",
    "VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_REFUSED",
    "VALIDATION_STATUS_INSERT_IMAGE_REQUEST_REFUSED",
    "VALIDATION_STATUS_INSERT_IMAGE_SOURCE_REFUSED",
    ...
    "HwpxInsertImageArtifact",
    "HwpxInsertImageReceipt",
    "HwpxInsertImageResult",
...
#: Largest caller-supplied image this facade forwards. The canonical PNG the
#: Core authority actually embeds is bounded by its own per-entry archive bound
#: (re-exported here as :data:`MAX_INSERT_IMAGE_BYTES`); this pre-check keeps a
#: caller from handing the Core image authority an unbounded payload it would
#: only refuse after a full decode.
MAX_INSERT_IMAGE_BYTES = MAX_HWPX_IMAGE_BYTES

#: Bounded ``hwpx.insert_image`` refusal vocabulary. Each code names the stage
#: that refused; that stage's own bounded code travels in the receipt ``note``.
REASON_INSERT_IMAGE_GATE_REJECTED = "insert_image_gate_rejected"
REASON_INSERT_IMAGE_SOURCE_REJECTED = "insert_image_source_rejected"
REASON_INSERT_IMAGE_REQUEST_SHAPE = "insert_image_request_shape"
REASON_INSERT_IMAGE_SECTION_INDEX_NEGATIVE = "insert_image_section_index_negative"
REASON_INSERT_IMAGE_EXTENT_TYPE = "insert_image_extent_type"
REASON_INSERT_IMAGE_IMAGE_BYTES_INVALID = "insert_image_image_bytes_invalid"
REASON_INSERT_IMAGE_IMAGE_BYTES_LIMIT = "insert_image_image_bytes_limit"
REASON_INSERT_IMAGE_OUTPUT_GATE_REJECTED = "insert_image_output_gate_rejected"
REASON_INSERT_IMAGE_OUTPUT_VALIDATE_REJECTED = "insert_image_output_validate_rejected"
REASON_INSERT_IMAGE_OUTPUT_READBACK_REJECTED = "insert_image_output_readback_rejected"
REASON_INSERT_IMAGE_OUTPUT_COUNT_DRIFT = "insert_image_output_count_drift"
REASON_INSERT_IMAGE_OUTPUT_PICTURE_MISMATCH = "insert_image_output_picture_mismatch"
REASON_INSERT_IMAGE_OUTPUT_NON_TARGET_DRIFT = "insert_image_output_non_target_drift"

#: Post-insert verification outcome. ``readback_ok`` is the only value that may
#: accompany a successful insertion: the common gate admitted the output, the
#: existing validate and read authorities accepted it, the inserted picture read
#: back through the single Core fact layer at the requested address and extent,
#: and every non-target member payload was proven byte-identical to the source.
VALIDATION_STATUS_INSERT_IMAGE_OK = "readback_ok"
VALIDATION_STATUS_INSERT_IMAGE_NOT_RUN = "not_run"
VALIDATION_STATUS_INSERT_IMAGE_SOURCE_REFUSED = "source_refused"
VALIDATION_STATUS_INSERT_IMAGE_REQUEST_REFUSED = "request_refused"
VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_REFUSED = "output_refused"
VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_MISMATCH = "output_mismatch"

_INSERT_IMAGE_VALIDATION_STATUSES = frozenset(
    {
        VALIDATION_STATUS_INSERT_IMAGE_OK,
        VALIDATION_STATUS_INSERT_IMAGE_NOT_RUN,
        VALIDATION_STATUS_INSERT_IMAGE_SOURCE_REFUSED,
...
ACCEPTANCE: dict[str, str] = {
    ...
    # Image insertion (#2825) embeds one PNG-canonical picture into a
    # manifest-carrying package at an exact section address, over the same
    # reserved edit capability. It is a bounded slice: no replace/delete, no
    # resize/crop/rotate, no direct JPEG embedding, no alignment or layout.
    "HWPX_INSERT_IMAGE_FACADE": "PASS",
    "IMAGE_INSERT": "PASS",
    "IMAGE_INSERT_SCOPE": "BOUNDED_PNG_CANONICAL_MANIFEST_PACKAGE",
    "IMAGE_INSERT_UNDER_HWPX_EDIT": "YES",
    "SINGLE_HWPX_IMAGE_INSERTION_AUTHORITY": "YES",
    "CORE_HWPX_IMAGE_INSERTION_REUSED": "YES",
    "SECOND_HWPX_IMAGE_AUTHORITY": "0",
    "SECOND_IMAGE_DECODER_AUTHORITY": "0",
    "CANONICAL_PNG_ONLY": "YES",
    "DIRECT_JPEG_EMBEDDING": "NOT_CLAIMED",
    "CALLER_MEMBER_OR_RELATIONSHIP_INPUT": "0",
    "INSERT_IMAGE_SUCCESS_REQUIRES_READBACK": "YES",
    "NON_TARGET_MEMBERS_BYTE_IDENTICAL": "YES",
...
# --------------------------------------------------------------------------
# hwpx.insert_image (#2825): bounded PNG-canonical embedding through the single
# Core image insertion authority
# --------------------------------------------------------------------------


@dataclass(frozen=True, slots=True)
class HwpxImageInsertionRequest:
    """One bounded request to embed one image into one section.

    ``section_index`` is **zero-based** and addresses the same ordered section
    list the Core image authority resolves against. ``image`` is raw PNG or JPEG
    bytes; the format is observed by Core's image authority, never declared
    here, so a caller cannot claim one encoding and supply another.
    ``width_hwpunit``/``height_hwpunit`` are the optional draw extent in HWPUNIT
    (1/7200 inch); when either is omitted Core derives it from the decoded
    image geometry.

    The request deliberately has no field for a manifest item id, relationship
    id, member name, member path, namespace, shape or style id: those stay
    outside caller authority. Like the edit request, it carries caller data and
    enforces nothing itself — :func:`hwpx_insert_image` is the single place the
    bounded contract is judged.
    """
...
@dataclass(frozen=True, slots=True)
class HwpxInsertImageReceipt:
    """Bounded ``hwpx.insert_image`` projection.

    On success it reports presentation metadata, the resulting structure counts,
    the inserted picture's draw extent and pixel geometry, and the post-insert
    verification outcome. On refusal every artifact-derived field is ``None``,
    so a refused insertion can never be read as a handoff.

    It carries bounded metadata and bounded reason vocabulary only — never the
    caller's image bytes, the canonical PNG payload, raw XML, ZIP member names
    or host paths. ``binary_item_id_ref`` is the manifest item id the picture
    references, not a package path, so holding it grants no path authority. The
    inserted bytes live in :class:`HwpxInsertImageArtifact`, which stays outside
    ...
    section_count: int | None
    paragraph_count: int | None
    picture_count: int | None
    inserted_image_count: int | None
    binary_item_id_ref: str | None
    image_media_type: str | None
    image_source_format: str | None
    image_bytes: int | None
    image_width_hwpunit: int | None
    image_height_hwpunit: int | None
    ...
    def __post_init__(self) -> None:
        _require_receipt_fields(self.status, self.reason_code)
        if self.validation_status not in _INSERT_IMAGE_VALIDATION_STATUSES:
            raise ValueError("hwpx_skill: insert_image validation_status must be bounded")
        if self.note is not None and not is_bounded_reason_code(self.note):
            raise ValueError("hwpx_skill: an insert_image note must be a bounded identifier")
        if self.status == STATUS_OK:
            if self.validation_status != VALIDATION_STATUS_INSERT_IMAGE_OK:
                raise ValueError("hwpx_skill: an ok insert_image must verify its readback")
            if self.note is not None:
                raise ValueError("hwpx_skill: an ok insert_image receipt carries no note")
            if self.media_type != HWPX_MEDIA_TYPE:
                raise ValueError("hwpx_skill: an ok insert_image must report the HWPX media type")
            if not isinstance(self.suggested_filename, str) or not self.suggested_filename.endswith(
                HWPX_SUFFIX
            ):
                raise ValueError("hwpx_skill: an ok insert_image name must use the canonical suffix")
            for name, value in (
                ("byte_size", self.byte_size),
                ("section_count", self.section_count),
                ("paragraph_count", self.paragraph_count),
                ("picture_count", self.picture_count),
                ("inserted_image_count", self.inserted_image_count),
                ("image_bytes", self.image_bytes),
                ("image_width_hwpunit", self.image_width_hwpunit),
                ("image_height_hwpunit", self.image_height_hwpunit),
                ("image_width_px", self.image_width_px),
                ("image_height_px", self.image_height_px),
            ):
                if not isinstance(value, int) or value <= 0:
                    raise ValueError(f"hwpx_skill: an ok insert_image must report {name}")
            if not is_hwpx_binary_item_id(self.binary_item_id_ref):
                raise ValueError(
                    "hwpx_skill: an ok insert_image must report the inserted manifest item id"
                )
            if not isinstance(self.image_media_type, str) or not self.image_media_type:
                raise ValueError("hwpx_skill: an ok insert_image must report the embedded media type")
            if not isinstance(self.image_source_format, str) or not self.image_source_format:
                raise ValueError("hwpx_skill: an ok insert_image must report the source format")
            return
        if self.validation_status == VALIDATION_STATUS_INSERT_IMAGE_OK:
            raise ValueError("hwpx_skill: a refused insert_image must not claim a verified readback")
        if any(
            value is not None
            for value in (
                self.media_type,
                ...
                self.paragraph_count,
                self.picture_count,
                self.inserted_image_count,
                self.binary_item_id_ref,
                self.image_media_type,
                self.image_source_format,
                self.image_bytes,
                self.image_width_hwpunit,
                self.image_height_hwpunit,
                self.image_width_px,
                self.image_height_px,
            )
        ):
            raise ValueError("hwpx_skill: a refused insert_image receipt carries no artifact metadata")
...
@dataclass(frozen=True, slots=True)
class HwpxInsertImageArtifact:
    """In-memory artifact for one verified image insertion.

    This is the only place an inserted package's bytes exist. It is deliberately
    separate from :class:`HwpxInsertImageReceipt` so no public projection can
    carry the payload, and nothing here is written to the host filesystem.
    """
    ...
    paragraph_count: int
    picture_count: int
    inserted_image_count: int
    binary_item_id_ref: str
    image_media_type: str
    image_source_format: str
    image_bytes: int
    image_width_hwpunit: int
    image_height_hwpunit: int
    image_width_px: int
    image_height_px: int

    def __post_init__(self) -> None:
        if not isinstance(self.payload, bytes) or not self.payload:
            raise ValueError("hwpx_skill: an insert_image artifact requires bounded bytes")
        if self.media_type != HWPX_MEDIA_TYPE:
            raise ValueError("hwpx_skill: an insert_image artifact must be the HWPX media type")
        if not isinstance(self.suggested_filename, str) or not self.suggested_filename.endswith(
            HWPX_SUFFIX
        ):
            raise ValueError("hwpx_skill: an insert_image artifact name must use the canonical suffix")
        if self.byte_size != len(self.payload):
            raise ValueError("hwpx_skill: an insert_image artifact byte size must match its payload")
        for name, value in (
            ("section_count", self.section_count),
            ("paragraph_count", self.paragraph_count),
            ("picture_count", self.picture_count),
            ...
            if not isinstance(value, int) or value <= 0:
                raise ValueError(f"hwpx_skill: an insert_image artifact must report {name}")
        if not is_hwpx_binary_item_id(self.binary_item_id_ref):
            raise ValueError("hwpx_skill: an insert_image artifact must report the item id")
        if not isinstance(self.image_media_type, str) or not self.image_media_type:
            raise ValueError("hwpx_skill: an insert_image artifact must report the embedded media type")
        if not isinstance(self.image_source_format, str) or not self.image_source_format:
            raise ValueError("hwpx_skill: an insert_image artifact must report the source format")


@dataclass(frozen=True, slots=True)
class HwpxInsertImageResult:
    """One insert_image outcome: the public receipt plus, on success, the bytes.

    ``to_public_dict`` delegates to the receipt, so the public projection of an
    insertion is exactly the bounded receipt and can never include the payload.
    A successful receipt always carries its artifact and a refused receipt never
    does, which keeps "no artifact" and "no success" the same fact.
    """

    receipt: HwpxInsertImageReceipt
    artifact: HwpxInsertImageArtifact | None = None

    def __post_init__(self) -> None:
        if self.receipt.status == STATUS_OK and self.artifact is None:
            raise ValueError("hwpx_skill: an ok insert_image result must carry an artifact")
        if self.receipt.status != STATUS_OK and self.artifact is not None:
            raise ValueError("hwpx_skill: a refused insert_image result must not carry an artifact")

    def to_public_dict(self) -> dict[str, object]:
        return self.receipt.to_public_dict()


def _insert_image_refusal(
    reason_code: str,
    *,
    note: str | None,
    validation_status: str,
) -> HwpxInsertImageResult:
    return HwpxInsertImageResult(
        receipt=HwpxInsertImageReceipt(
            status=STATUS_REFUSED,
            capability_id=CAPABILITY_HWPX_EDIT,
            reason_code=reason_code,
...
#: Core image-authority refusals that are about the *request* rather than about
#: the supplied package: the instruction container, the address, the draw
#: extent, the supplied payload and the id space. Everything else the authority
#: raises describes the source package, so an unlisted code is a source refusal
#: by default rather than being silently reclassified as the caller's fault.
_INSERT_IMAGE_REQUEST_CODES = frozenset(
    {
        "hwpx_image_request",
        "hwpx_image_limit",
...
def _insert_image_request_refusal(request: object) -> str | None:
    """First bounded refusal for the request container's own shape, or None.

    Judged only after the common gate admitted the source bytes, so a malformed
    request cannot return ahead of the archive/path/expansion gate. ``bool`` is
    rejected explicitly even though it is an ``int`` subclass, so ``True`` can
    never address section 1.
    """

    if not isinstance(request, HwpxImageInsertionRequest):
        return REASON_INSERT_IMAGE_REQUEST_SHAPE
    if not isinstance(request.section_index, int) or isinstance(request.section_index, bool):
        return REASON_INSERT_IMAGE_REQUEST_SHAPE
    if request.section_index < 0:
        return REASON_INSERT_IMAGE_SECTION_INDEX_NEGATIVE
    for value in (request.width_hwpunit, request.height_hwpunit):
        if value is not None and (isinstance(value, bool) or not isinstance(value, int)):
            return REASON_INSERT_IMAGE_EXTENT_TYPE
    if not isinstance(request.image, (bytes, bytearray)):
        return REASON_INSERT_IMAGE_IMAGE_BYTES_INVALID
    if not request.image:
        return REASON_INSERT_IMAGE_IMAGE_BYTES_INVALID
    if len(request.image) > MAX_INSERT_IMAGE_BYTES:
        return REASON_INSERT_IMAGE_IMAGE_BYTES_LIMIT
    return None


def _insert_image_source_refusal(error: DocumentNormalizationError) -> HwpxInsertImageResult:
    """Project a Core image-authority refusal onto one bounded facade refusal."""

    code = getattr(error, "code", None)
    if code in _INSERT_IMAGE_REQUEST_CODES:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_SOURCE_REJECTED,
            note=_bounded_note(code),
            validation_status=VALIDATION_STATUS_INSERT_IMAGE_REQUEST_REFUSED,
        )
    return _insert_image_refusal(
        REASON_INSERT_IMAGE_SOURCE_REJECTED,
        note=_bounded_note(code),
        validation_status=VALIDATION_STATUS_INSERT_IMAGE_SOURCE_REFUSED,
    )


def _insert_image_picture_count(facts: tuple[object, ...]) -> int:
    return sum(
        block.kind == "picture"
        for section in facts
        for block in section.blocks  # type: ignore[attr-defined]
    )


def _insert_image_paragraph_count(facts: tuple[object, ...]) -> int:
    return sum(
...
def _verify_insert_image_preservation(source: bytes, inserted: bytes) -> str | None:
    """First bounded preservation refusal, or None once the proof holds.

    The claim is deliberately narrow and is made through Core accessors only.
    The member-name sequence must be the source sequence plus exactly one new
    ``BinData`` member appended at the end, the manifest must have changed, at
    most two member payloads — the manifest and the addressed section part — may
    differ from the source, and every other payload must be byte-identical. That
    is the largest set of bytes a single anchored manifest-plus-section splice
    can touch, so a wider change is refused rather than accepted as harmless.
    """

    try:
        before = read_hwpx_package_members(source)
        after = read_hwpx_package_members(inserted)
    except DocumentNormalizationError:
        return REASON_INSERT_IMAGE_OUTPUT_NON_TARGET_DRIFT
    if len(after) != len(before) + 1:
        return REASON_INSERT_IMAGE_OUTPUT_COUNT_DRIFT
    if [member.name for member in after[: len(before)]] != [member.name for member in before]:
        return REASON_INSERT_IMAGE_OUTPUT_NON_TARGET_DRIFT
    added = after[-1]
    if not added.name.startswith(HWPX_BIN_DATA_PREFIX) or added.name in {
        member.name for member in before
    }:
        return REASON_INSERT_IMAGE_OUTPUT_NON_TARGET_DRIFT
    source_by_name = {member.name: member.payload for member in before}
    changed = [
        member.name
        for member in after
        if member.name in source_by_name and source_by_name[member.name] != member.payload
    ]
    if HWPX_MANIFEST_MEMBER not in changed or len(changed) > 2:
        return REASON_INSERT_IMAGE_OUTPUT_NON_TARGET_DRIFT
    return None


def hwpx_insert_image(
    filename: str,
    payload: bytes,
    request: HwpxImageInsertionRequest,
) -> HwpxInsertImageResult:
    """Embed one bounded image into an existing manifest-carrying HWPX package.

    Composition order, with the common gate always first::

        caller filename + package bytes + one bounded image request
        -> inspect_file()                       (common gate, HWPX_CANDIDATE)
        -> the request container, index type, draw extent and payload judged
        -> insert_hwpx_image()                   (the single Core image authority)
        -> inspect_file() / hwpx_validate() / hwpx_read() on the output
        -> read_hwpx_section_facts() on the output
        -> the inserted picture read back at the requested address and extent
        -> member-sequence and non-target payload preservation proof
        -> bounded insert_image receipt + in-memory artifact

    The ordering is part of the contract: the common file-intake authority
    decides first, and the request is judged only after the source has been
    admitted, so a malformed request cannot return ahead of the gate that
    inspects the source bytes.

    Why the Core authority and not a facade-side splice: the canonical PNG
    decision, the ``BIN####`` id allocation and both byte splices belong to
    ``padiem_ai_core.hwpx_image_insertion``. Re-implementing any of them here
    would create a second image decoder, a second manifest writer or a second
    archive writer, which is exactly what this facade exists to avoid. What the
    facade adds is admission, bounded judging, and a receipt that only reports
    success once the output re-enters the accepted read, validate and readback
    authorities.

    Success is proven by readback, not by the Core authority's own bookkeeping:
    the inserted picture must be found again through the single Core fact layer
    at the requested section with the requested draw extent, and every member
    payload the splice cannot have touched must be byte-identical to the source.
    Any failure at any stage is a bounded refusal with no artifact and no
    fallback format.
    ...
    # 2. Only now is the request judged, against an admitted source.
    request_refusal = _insert_image_request_refusal(request)
    if request_refusal is not None:
        return _insert_image_refusal(
            request_refusal,
            note=None,
    ...
    if not isinstance(request, HwpxImageInsertionRequest):  # pragma: no cover - judged above
        raise AssertionError("hwpx_skill: the request shape was already judged")

    # 3. The single Core image authority. It owns source admission, the manifest
    #    shape, the canonical PNG decision, id allocation and both splices, and
    #    it raises one bounded DocumentNormalizationError per refusal.
    insertion = HwpxImageInsertion(
        section_index=request.section_index,
        image=bytes(request.image),
        width_hwpunit=request.width_hwpunit,
        height_hwpunit=request.height_hwpunit,
    )
    try:
        inserted, core_receipts = insert_hwpx_image(payload, (insertion,))
    except DocumentNormalizationError as error:
        return _insert_image_source_refusal(error)
    if not isinstance(inserted, bytes) or not inserted or len(core_receipts) != 1:
        # Defence in depth: a Core insertion that returns no bounded byte string
        # or an unpaired receipt can never produce a facade success.
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_SOURCE_REJECTED,
            note=None,
            validation_status=VALIDATION_STATUS_INSERT_IMAGE_SOURCE_REFUSED,
        )
    (core_receipt,) = core_receipts

    name = _suggested_filename(filename)

    # 4. Re-enter the accepted Skill authorities on the output.
    output_gate = inspect_file(name, inserted)
    output_gate_refusal = _gate_refusal(output_gate)
    if output_gate_refusal is not None:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_GATE_REJECTED,
            note=_bounded_note(output_gate_refusal),
            validation_status=VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_REFUSED,
        )

    validation = hwpx_validate(name, inserted)
    if validation.status != STATUS_OK:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_VALIDATE_REJECTED,
            note=_bounded_note(validation.reason_code),
    ...
    # 5. Readback through the single Core fact layer. The ids and extents the
    #    Core receipt claims are the ones the reader must actually find.
    try:
        source_facts = read_hwpx_section_facts(payload)
        output_facts = read_hwpx_section_facts(inserted)
    except DocumentNormalizationError as error:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_READBACK_REJECTED,
            note=_bounded_note(getattr(error, "code", None)),
    ...
    if len(output_facts) != len(source_facts):
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_COUNT_DRIFT,
            note=None,
            validation_status=VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_MISMATCH,
        )
    if request.section_index >= len(output_facts):
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_COUNT_DRIFT,
            note=None,
            validation_status=VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_MISMATCH,
        )
    if sum(len(section.blocks) for section in output_facts) != sum(
        len(section.blocks) for section in source_facts
    ) + 1:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_COUNT_DRIFT,
            note=None,
    ...
    if _insert_image_picture_count(output_facts) != _insert_image_picture_count(source_facts) + 1:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_PICTURE_MISMATCH,
            note=None,
            validation_status=VALIDATION_STATUS_INSERT_IMAGE_OUTPUT_MISMATCH,
    ...
    found = [
        block.picture
        for block in output_facts[request.section_index].blocks
        if block.kind == "picture" and block.picture is not None
    ]
    matching = [
        picture
        for picture in found
        if picture.binary_item_id_ref == core_receipt.binary_item_id_ref
        and picture.width_hwpunit == core_receipt.width_hwpunit
        and picture.height_hwpunit == core_receipt.height_hwpunit
        and not picture.unsupported
    ]
    if len(matching) != 1:
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_PICTURE_MISMATCH,
    ...
    if any(
        section.structured_unsupported_nodes != source.structured_unsupported_nodes
        for section, source in zip(output_facts, source_facts, strict=True)
    ):
        return _insert_image_refusal(
            REASON_INSERT_IMAGE_OUTPUT_NON_TARGET_DRIFT,
            note=None,
    ...
    # 6. Preservation of every member the splice cannot have touched.
    drift = _verify_insert_image_preservation(payload, inserted)
    ...
    section_count = len(output_facts)
    paragraph_count = _insert_image_paragraph_count(output_facts)
    picture_count = _insert_image_picture_count(output_facts)
    artifact = HwpxInsertImageArtifact(
        payload=inserted,
        media_type=HWPX_MEDIA_TYPE,
        suggested_filename=name,
        byte_size=len(inserted),
        section_count=section_count,
        paragraph_count=paragraph_count,
        picture_count=picture_count,
        inserted_image_count=1,
        binary_item_id_ref=core_receipt.binary_item_id_ref,
        image_media_type=core_receipt.media_type,
        image_source_format=core_receipt.source_format,
        image_bytes=core_receipt.image_bytes,
        image_width_hwpunit=core_receipt.width_hwpunit,
        image_height_hwpunit=core_receipt.height_hwpunit,
        image_width_px=core_receipt.width_px,
        image_height_px=core_receipt.height_px,
    )
    return HwpxInsertImageResult(
        receipt=HwpxInsertImageReceipt(
            status=STATUS_OK,
