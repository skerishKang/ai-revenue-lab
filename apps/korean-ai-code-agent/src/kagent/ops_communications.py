...
class CommunicationSendRequest:
    ...
    def safe_metadata(self) -> dict[str, Any]:
        """Log-safe metadata; deliberately omits message body and subject text."""
        return {
...
class InboundCommunication:
    ...
    def safe_metadata(self) -> dict[str, Any]:
        """Never expose full untrusted message content in general audit/log projections."""
        return {
...
class DeterministicFakeCommunicationConnector:
    """Network-free test adapter only."""
    ...
    def send(self, request: CommunicationSendRequest) -> CommunicationDeliveryReceipt:
        ...
        digest = hashlib.sha256(
            f"{request.request_id}:{request.action_fingerprint}".encode("utf-8")
        ).hexdigest()[:24]
