"""httpx Gmail read port for the Engine Worker (WO-10 PR-C2, D28).
...
provider GET; the trusted :class:`padiem_ai_core.connectors.GmailReadPort`
...
ALLOWED_GMAIL_HOSTS = frozenset({"gmail.googleapis.com", "oauth2.googleapis.com"})
...
class HttpxGmailReadPort(GmailReadPort):
    """Fetch-based Gmail read port over httpx with OAuth refresh.

    Constructed only when all three Worker secrets are present; the
    composition root (``worker_identity._gmail_port_for_env``) refuses to
    ...
    async def _refresh(self) -> str:
        ...
        if (
            ...
            or GMAIL_READONLY_SCOPE not in returned_scopes.split()
        ):
    ...
    async def get_json(
        ...
        *,
        ...
        if not set(required_scopes) <= {GMAIL_READONLY_SCOPE}:
