...
class GitHubLiveStaticContractTests(unittest.TestCase):
    ...
    def test_server_uses_fixed_graphql_origin_and_query(self):
        ...
        self.assertIn("statusCheckRollup", client)
        self.assertRegex(client, r"issues\(first:\s*1,\s*states:\s*OPEN\)")
        self.assertRegex(client, r"pullRequests\(first:\s*1,\s*states:\s*OPEN\)")
        self.assertRegex(client, r"commits\(last:\s*1\)")
        self.assertRegex(client, r"contexts\(first:\s*100\)")
        self.assertRegex(client, r"search\(query:\s*\$draftQuery,\s*type:\s*ISSUE,\s*first:\s*1\)")
        self.assertNotIn("issues(states: OPEN)", client)
        self.assertNotIn("pullRequests(states: OPEN)", client)
        self.assertIn("`issue${mapping.issueNumber}: issue(number: ${mapping.issueNumber})", client)
        self.assertIn("`pr${mapping.pullRequestNumber}: pullRequest(number: ${mapping.pullRequestNumber})", client)
