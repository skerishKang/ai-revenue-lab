...
class GitHubLiveStaticContractTests(unittest.TestCase):
    ...
    def test_routes_only_include_api(self):
        self.assertEqual(json.loads((ROOT / "_routes.json").read_text(encoding="utf-8")), {"version": 1, "include": ["/api/*"], "exclude": []})
    ...
    def test_server_uses_fixed_graphql_origin_and_query(self):
        ...
        query_builder = (FUNCTIONS / "_lib" / "business-github-query.js").read_text(encoding="utf-8")
        ...
        self.assertIn("fallbackPr${n}: pullRequest(number: ${n})", query_builder)
        self.assertIn("prSearchMarker${m.number}_${phase}", query_builder)
        self.assertIn("prSearchConvention${m.number}_${phase}", query_builder)
        self.assertIn("head:business-${m.number}-${phase}", query_builder)
    ...
    def test_kv_is_access_compatible_authoritative_cache(self):
        ...
        cache = (FUNCTIONS / "_lib" / "cache.js").read_text(encoding="utf-8")
        self.assertIn("GITHUB_STATUS_SNAPSHOT_KV", route)
        self.assertIn("cacheConfigurationMissingPayload", route)
        self.assertIn("CACHE_CONFIGURATION_MISSING", (FUNCTIONS / "_lib" / "response.js").read_text(encoding="utf-8"))
    ...
    def test_token_and_refresh_single_flight_are_explicit(self):
        auth = (FUNCTIONS / "_lib" / "github-app-auth.js").read_text(encoding="utf-8")
        service = (FUNCTIONS / "_lib" / "github-status-service.js").read_text(encoding="utf-8")
    ...
    def test_rate_limit_is_normalized_without_general_retry(self):
        ...
        self.assertIn('GITHUB_GRAPHQL_PARTIAL_RESPONSE', (FUNCTIONS / "_lib" / "response.js").read_text(encoding="utf-8"))
