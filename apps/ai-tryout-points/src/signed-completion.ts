...
import { ACTION_TYPES, COMPLETION_EVENT_TYPES, type SignedCompletionEvent } from './domain.js';
...
export function verifySignedCompletionEvent(input: CompletionVerificationInput): CompletionVerificationDecision {
  if (!input.providerId || !input.rawBody || !input.secret || Buffer.byteLength(input.rawBody, 'utf8') > MAX_SIGNED_BODY_BYTES || !/^\d{13}$/.test(input.signedTimestamp)) {
  ...
  if (Number.isNaN(timestamp) || Number.isNaN(now) || !Number.isSafeInteger(maxAge) || maxAge <= 0 || maxAge > 24 * 60 * 60 * 1000) return { accepted: false, reason: 'INVALID_TIMESTAMP' };
  if (Math.abs(now - timestamp) > maxAge) return { accepted: false, reason: 'STALE_TIMESTAMP' };
