...
export const P2_OPPORTUNITY_KINDS = [
  'MICROTASK',
  'DATA_TASK',
  'EVALUATION_TASK',
  'TRANSCRIPTION_TASK',
] as const;
export type P2OpportunityKind = (typeof P2_OPPORTUNITY_KINDS)[number];

export type P2SupplyMode =
  | 'CURRENT_TASK_INVENTORY'
  | 'PUBLIC_PROJECT_APPLICATION'
  | 'PROGRAM_REFERENCE'
  | 'UNKNOWN_SUPPLY';

export type P2PreparationState =
  | 'RANKABLE'
  | 'PROJECT_APPLICATION_REFERENCE_ONLY'
  | 'PROGRAM_REFERENCE_ONLY'
  | 'UNRANKABLE_MISSING_CURRENT_TASK_SUPPLY'
  | 'UNRANKABLE_MISSING_CRITICAL_DATA'
  | 'BLOCKED_MONETARY_PREREQUISITE'
  | 'BLOCKED_TRUST'
  | 'BLOCKED_INACTIVE';

export type P2PayModel =
  | 'HOURLY'
  | 'FIXED_PROJECT'
  | 'PER_TASK'
  | 'PER_UNIT'
...
export interface P2PreparedOpportunity {
  readonly opportunityId: string;
  readonly canonicalKey: string;
  readonly sourceId: string;
  readonly title: string;
  readonly kind: P2OpportunityKind;
  readonly preparationState: P2PreparationState;
  readonly supplyMode: P2SupplyMode;
  readonly payModel: P2PayModel;
...
export const P2_VISIBILITY_LOCK = Object.freeze({
  issueNumber: 1133 as const,
  ...
  unlockAuthority: 'SEPARATE_OWNER_CENTRAL_DECISION_AFTER_P0_P1_SEQUENCE' as const,
});

const P2_CATEGORIES = new Set<OpportunityCategory>([
  'MICROTASK',
  'DATA_ANNOTATION',
  'DATA_REVIEW',
...
const CURRENT_TASK_SUPPLY_STATES = new Set([
  'AVAILABLE',
  'LIVE',
  'CURRENT_TASK_AVAILABLE',
  'PUBLIC_TASK_AVAILABLE',
]);

function isP2Candidate(record: VerifiedOpportunityTrustRecord): boolean {
  if (!P2_CATEGORIES.has(record.version.opportunityCategory)) return false;
  return record.version.incomeLadderLevel === 'MICRO_REWARD' || record.version.incomeLadderLevel === 'TASK_WORK';
}

function kindFor(record: VerifiedOpportunityTrustRecord): P2OpportunityKind {
...
function supplyModeFor(record: VerifiedOpportunityTrustRecord): P2SupplyMode {
  if (record.supplyClaimMode === 'PROVIDER_PROGRAM_ONLY') return 'PROGRAM_REFERENCE';
  if (record.version.supplyAvailabilityState !== null && CURRENT_TASK_SUPPLY_STATES.has(record.version.supplyAvailabilityState)) {
    return 'CURRENT_TASK_INVENTORY';
  }
  const state = record.version.supplyAvailabilityState ?? '';
  if (record.version.applicationRequired === true && (state.includes('PROJECT') || state.includes('APPLICATION'))) {
    return 'PUBLIC_PROJECT_APPLICATION';
  }
  return 'UNKNOWN_SUPPLY';
}

function payModelFor(record: VerifiedOpportunityTrustRecord): P2PayModel {
...
function knownFrictionScore(record: VerifiedOpportunityTrustRecord): number {
  let score = 0;
  if (record.version.applicationRequired === true) score += 3;
  if (record.version.qualificationRequired === true) score += 3;
  ...
  if (record.version.taxContractorRequirements !== null) score += 1;
  if (record.version.payoutDelay !== null) score += 1;
  if ((record.version.startLatencyMinutes ?? 0) > 0) score += 1;
  score += Math.min(5, record.requirements.filter((requirement) => requirement.required).length);
  return score;
}

function stateFor(record: VerifiedOpportunityTrustRecord): P2PreparationState {
  ...
  if (supplyMode !== 'CURRENT_TASK_INVENTORY') return 'UNRANKABLE_MISSING_CURRENT_TASK_SUPPLY';
...
export function prepareP2Opportunity(record: VerifiedOpportunityTrustRecord): P2PreparedOpportunity | null {
  if (!isP2Candidate(record)) return null;
...
const CERTAINTY_RANK: Record<P2PreparedOpportunity['certainty'], number> = {
  GUARANTEED: 0,
  CONDITIONAL: 1,
  DRAW: 2,
};
const STATE_RANK: Record<P2PreparationState, number> = {
  RANKABLE: 0,
  PROJECT_APPLICATION_REFERENCE_ONLY: 1,
  PROGRAM_REFERENCE_ONLY: 2,
  UNRANKABLE_MISSING_CURRENT_TASK_SUPPLY: 3,
  UNRANKABLE_MISSING_CRITICAL_DATA: 4,
  BLOCKED_MONETARY_PREREQUISITE: 5,
  BLOCKED_TRUST: 6,
  BLOCKED_INACTIVE: 7,
};

export function compareP2PreparedOpportunities(a: P2PreparedOpportunity, b: P2PreparedOpportunity): number {
...
export interface P2PreparedBacklogViewModel {
  readonly mode: 'P2_MICROTASK_PREPARATION_HIDDEN';
  readonly issueNumber: 1133;
  readonly consumerVisible: false;
  readonly visibilityLock: typeof P2_VISIBILITY_LOCK;
  readonly opportunities: readonly P2PreparedOpportunity[];
...
export function buildP2PreparedBacklog(records: readonly VerifiedOpportunityTrustRecord[]): P2PreparedBacklogViewModel {
  const sorted = records
    .map(prepareP2Opportunity)
    .filter((item): item is P2PreparedOpportunity => item !== null)
    .sort(compareP2PreparedOpportunities);

  const deduped: P2PreparedOpportunity[] = [];
  ...
  return Object.freeze({
    mode: 'P2_MICROTASK_PREPARATION_HIDDEN' as const,
    issueNumber: 1133 as const,
    consumerVisible: false as const,
    visibilityLock: P2_VISIBILITY_LOCK,
    opportunities,
