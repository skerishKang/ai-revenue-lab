(() => {
  ...
  function renderAnswer(article, result) {
    ...
    article.querySelector("[data-runtime-label]").textContent = result.runtime === "mock" ? "모의 응답 · 실제 모델 호출 없음" : "AI 응답";
  ...
  function renderError(article, message, retryMessages) {
    const content = article.querySelector(".assistant-content"); content.replaceChildren();
    ...
    retry.addEventListener("click", () => { article.remove(); requestAnswer(retryMessages); }, { once: true });
    box.append(strong, p, retry); content.appendChild(box);
  }
  async function requestAnswer(outboundMessages) {
    if (inFlight) return; inFlight = true; updateComposer();
    const article = addAssistantShell("답변 준비 중"); renderTyping(article);
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: outboundMessages, mode: "auto" }) });
      ...
      renderError(article, error instanceof Error ? error.message : "다시 시도해 주세요.", outboundMessages);
  ...
  function resetChat() { messages = []; messageList.replaceChildren(); messageList.hidden = true; emptyState.hidden = false; shell.dataset.state = "home"; input.value = ""; updateComposer(); closeSidebar(); input.focus(); }
  ...
  document.querySelectorAll("[data-prompt]").forEach((button) => button.addEventListener("click", () => { submitPrompt(button.dataset.prompt || ""); closeSidebar(); }));
