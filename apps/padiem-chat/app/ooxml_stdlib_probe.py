...
def _open_bounded_archive(payload: bytes) -> ZipFile:
    if not isinstance(payload, (bytes, bytearray)):
        raise OOXMLCompatibilityError("OOXML payload must be bytes")
    if not payload or len(payload) > MAX_ARCHIVE_BYTES:
        raise OOXMLCompatibilityError("OOXML archive size out of bounds")

    try:
        archive = ZipFile(BytesIO(bytes(payload)))
        infos = archive.infolist()
    except BadZipFile as exc:
        raise OOXMLCompatibilityError("malformed OOXML ZIP archive") from exc

    try:
        if len(infos) > MAX_ENTRIES:
            raise OOXMLCompatibilityError("OOXML archive has too many entries")

        total_uncompressed = 0
        for info in infos:
            _reject_unsafe_member_name(info.filename)
            if info.flag_bits & 0x1:
                raise OOXMLCompatibilityError("encrypted OOXML entries are not supported")
            if info.file_size > MAX_ENTRY_UNCOMPRESSED_BYTES:
                raise OOXMLCompatibilityError("OOXML entry exceeds size limit")
            total_uncompressed += info.file_size
            if total_uncompressed > MAX_TOTAL_UNCOMPRESSED_BYTES:
                raise OOXMLCompatibilityError("OOXML archive exceeds total uncompressed limit")
    except Exception:
        archive.close()
        raise
...
def _read_xml_part(archive: ZipFile, name: str) -> ET.Element:
    try:
        ...
        raise OOXMLCompatibilityError(f"missing OOXML part: {name}") from exc

    if b"<!DOCTYPE" in raw.upper():
        raise OOXMLCompatibilityError("OOXML DTDs are not supported")
    try:
        return ET.fromstring(raw)
    except ET.ParseError as exc:
        raise OOXMLCompatibilityError(f"invalid OOXML XML part: {name}") from exc


def _bounded_join(parts: list[str], separator: str = "\n") -> str:
    text = separator.join(part for part in parts if part)
    if len(text) > MAX_EXTRACTED_TEXT_CHARS:
        raise OOXMLCompatibilityError("extracted OOXML text exceeds limit")
    return text


def extract_docx_text_stdlib(payload: bytes) -> str:
...
def extract_pptx_text_stdlib(payload: bytes) -> str:
    archive = _open_bounded_archive(payload)
    try:
        ...
        if not slides:
            raise OOXMLCompatibilityError("missing PPTX slide XML parts")
