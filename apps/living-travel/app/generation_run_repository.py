"""Generation run repository for Living Travel."""
...
_COLS = [
    ...
    "error_category", "error_message", "edition_id", "success", "created_at",
]
_SELECT = ", ".join(_COLS)
...
def count_generation_runs_by_edition(
    conn: sqlite3.Connection, edition_id: str
) -> int:
    row = conn.execute(
        "SELECT COUNT(*) as cnt FROM generation_runs WHERE edition_id = ?",
        (edition_id,),
    ).fetchone()
