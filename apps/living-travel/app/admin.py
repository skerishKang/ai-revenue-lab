...
or any persisted report; only a generic confirmation is printed.
"""
    ...
    try:
        with conn:
            identity = eid_repo.ensure_identity(
                conn, PROVIDER_FIREBASE, firebase_uid, principal_type="operator"
            )
            ...
            if identity.is_revoked:
                ...
                return 1
            operator_id = f"op_{firebase_uid}"
            result = eid_repo.link_operator(conn, identity.id, operator_id)
            if result is None:
